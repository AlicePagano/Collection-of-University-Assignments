...
program demo
    ...
    call GET_COMMAND_ARGUMENT(3, value=debug_option)
    ...
    do tt=1,N_times
        ...
        call cpu_time(t1)
        CALL MatMultbyRow(A,B,C) ! Matrix-matrix multiplication by row
        call cpu_time(t2)

        dt = t2-t1 ! time of matrix-matrix multiplication by row
        time(tt) = dt
    end do
