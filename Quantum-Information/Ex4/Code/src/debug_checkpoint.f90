MODULE DEBUG_CHECKPOINT
    ...
    CONTAINS
        ...
        !
        SUBROUTINE do_checkpoint(debug,cp_name,file_name,line)
        	implicit none
        	logical, intent(in) :: debug
            character(len=*), intent(in), optional :: cp_name, file_name  ! (optional) checkpoint name
            integer(4), optional :: line
        ...
        !
        SUBROUTINE check_matrix(debug,MServ,cp_name,file_name,line)
            implicit none
            logical, intent(in) :: debug
            ...
            IF (debug) THEN
                call do_checkpoint(debug,cp_name,file_name,line)
